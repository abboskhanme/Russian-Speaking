...
class OrthoepyError(BaseModel):
    """A word the student read AS SPELLED, violating Russian orthoepy (written
    form ≠ spoken form). Detected from the AUDIO, not the transcript."""
    ...
    correct: str = Field(
        description="Правильное произношение в квадратных скобках (например: [малако́])"
    )
    said: str = Field(
        description="Как студент произнёс — неправильно, ПО НАПИСАНИЮ, в скобках (например: [молоко́])"
    )
...
class EvaluationResult(BaseModel):
    ...
    orthoepy_errors: list[OrthoepyError] = Field(
        description=(
            "Слова, которые студент прочитал ПО НАПИСАНИЮ, нарушив орфоэпию — "
            "услышанные в АУДИО. Пустой список, если таких нет или аудио отсутствует."
        )
    )
...
ОРФОЭПИЯ — ЧТЕНИЕ «ПО НАПИСАНИЮ» (orthoepy_errors): по АУДИО определи слова, которые \
студент произнёс так, КАК ОНИ ПИШУТСЯ, нарушив нормы устного русского (написание ≠ \
произношение). Типичные случаи:
- безударные «О»/«Е» редуцируются: молоко → [малако́], хорошо → [харашо́];
- звонкие согласные на конце оглушаются: друг → [друк], город → [го́рат];
- звонкая перед глухой оглушается: автобус → [афто́бус];
- «-ого»/«-его» в окончаниях: его → [йево́], красивого → [краси́вава];
- «что»/«чт» → [што]; «-тся»/«-ться» → [ца]; непроизносимые согласные: здравствуйте → \
[здра́ствуйте], солнце → [со́нце].
Для каждой ошибки: correct — ТОЧНАЯ орфоэпическая запись всего слова в [скобках]; said — \
как студент прочитал по буквам, в [скобках]. Правила: (1) отмечай ТОЛЬКО то, что реально \
СЛЫШНО в аудио как «чтение по буквам»; не выдумывай и не трогай верно произнесённые слова. \
(2) Пиши записи аккуратно и посимвольно — НЕ удваивай слоги, не добавляй/не теряй буквы, \
проверь, что запись читается как реальное слово; если не уверен в точной записи — пропусти \
слово. (3) Если аудио нет или всё произнесено правильно — верни ПУСТОЙ список. (4) rule_uz — \
на узбекском (O'zbekcha), rule_ru — на русском; все русские слова только кириллицей.
...
def analyze(
    *,
    ...
    user_content = (
        f"{context_block}{image_note}{audio_note}{instruction_block}"
        f"Текст задания (на которое отвечал студент):\n{question_prompt}\n\n"
        f"Транскрипция устного ответа студента:\n«{transcript_text}»\n\n"
        f"Оцени этот ответ согласно критериям. Учитывай, выполнил ли студент условие задания "
        f"(relevance/соответствие теме — насколько ответ отвечает условию)."
    )
...
# ─── Orthoepy-only pass (for shadowing, which skips the full analysis) ───────
...
студент читает русский текст вслух, и сам текст. Найди слова, которые студент произнёс \
ТАК, КАК ОНИ ПИШУТСЯ, нарушив нормы устного русского (написание ≠ произношение). Основные \
правила орфоэпии:
- безударные «О»/«Е» редуцируются → [а]/[и]: молоко→[малако́], хорошо→[хърашо́], сегодня→[сиво́дня];
- звонкие согласные на конце оглушаются: друг→[друк], город→[го́рът]; и перед глухими: автобус→[афто́бус];
- «-ого»/«-его» в окончаниях → [-ова]/[-ева]: его→[йево́], красивого→[краси́въва];
- «что»/«чт»→[што]; «-тся»/«-ться»→[ца]; непроизносимые согласные: здравствуйте→[здра́ствуйте], солнце→[со́нце].

Для каждой ошибки заполни поля:
- word — слово как в тексте; word_with_stress — то же с ударением;
- correct — ТОЧНАЯ орфоэпическая запись ВСЕГО слова в [квадратных скобках];
- said — как ошибочно прочитал студент (по буквам), в [скобках].

СТРОГО: correct и said пиши аккуратно и посимвольно; НЕ удваивай слоги, не добавляй и не \
теряй буквы; мысленно перечитай запись и проверь, что она читается как реальное слово. \
Отмечай ТОЛЬКО реально услышанное «чтение по буквам»; не выдумывай и не трогай верно \
произнесённые слова. Если ошибок нет, аудио неразборчиво ИЛИ ты не уверен в точной записи — \
НЕ включай слово (лучше пропустить, чем дать неверную запись). rule_uz — на узбекском \
...
def detect_orthoepy(
    *,
    ...
    """Listen to a read-aloud recording and flag words read AS SPELLED (orthoepy
    errors). Used for shadowing, which has no full LLM analysis. Best-effort:
    returns [] on any failure so it never breaks the pipeline."""
    if not audio_bytes:
        return []
    ref = (reference_text or transcript_text or "").strip()
    user_content = (
        (f"Целевой текст, который читал студент:\n«{ref}»\n\n" if ref else "")
...
def generate_model_answer(
    *,
    ...
    `instruction` is the task's condition ("shart") so the answer actually fits
    what was asked. `variety=True` raises temperature and asks for a fresh,
    different phrasing so repeated calls give a new variant each time."""
    ctx = []
    if question_title:
    ...
    if instruction and instruction.strip():
        ctx.append(f"Условие (что нужно сделать): {instruction.strip()}")
    block = ("\n".join(ctx) + "\n\n") if ctx else ""
    variety_note = (
        "\n\nДай СВЕЖИЙ, НОВЫЙ вариант — другие слова, обороты и порядок мыслей, "
        "чем в типичном/предыдущем ответе, но по тому же заданию."
        if variety
        else ""
    )
    user_content = f"{block}Задание:\n{question_prompt}\n\nНапиши образцовый ответ.{variety_note}"
    config = types.GenerateContentConfig(
        system_instruction=_MODEL_ANSWER_SYSTEM,
        temperature=1.15 if variety else None,
        thinking_config=types.ThinkingConfig(thinking_budget=0),
    )
    for attempt in range(_MAX_ATTEMPTS):
        try:
