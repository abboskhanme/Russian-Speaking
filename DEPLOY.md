...
## 10. Automatic zero-downtime updates (CI/CD)

The old redeploy (`docker compose up -d --build`) rebuilt both images **on the
B1S VM** — slow (2-10 min) and disruptive on 1 GB RAM. Instead, images are now
built in **GitHub Actions** and pushed to **GHCR**; the VM only *pulls* the
finished images and swaps the app containers (`deploy/deploy.sh`). A redeploy is
a ~15-30s swap, and `db`/`redis`/`minio` never restart.

**Flow:** `git push` to `main` → CI runs tests → builds `…-backend` + `…-web`
images (tagged `latest` + the commit SHA) → SSHes into the VM → runs
`deploy/deploy.sh` (pull → migrate → `up -d --no-deps backend worker web`).

### One-time setup

1. **GHCR image access on the VM.** Either make the two packages public
   (GitHub → your profile → Packages → `russian-speaking-backend` and
   `russian-speaking-web` → Package settings → Change visibility → Public), or
   log in once on the VM with a `read:packages` PAT:
   ```bash
   echo <GHCR_PAT> | docker login ghcr.io -u abboskhanme --password-stdin
   ```

2. **SSH key for CI.** Create a key pair, add the public key to the VM's
   `~/.ssh/authorized_keys`, keep the private key for the secret below:
   ```bash
   ssh-keygen -t ed25519 -f deploy_key -N ""   # deploy_key (private), deploy_key.pub (public)
   ```

3. **GitHub → repo Settings → Secrets and variables → Actions:**
   - **Secrets:** `DEPLOY_SSH_HOST` (VM IP or domain), `DEPLOY_SSH_USER` (e.g.
     `azureuser`), `DEPLOY_SSH_KEY` (the private `deploy_key`), and if Google
     sign-in is used, `VITE_GOOGLE_CLIENT_ID`. Optional: `DEPLOY_SSH_PORT`
     (default 22), `DEPLOY_DIR` (repo path on the VM, default `~/russpeak`).
   - **Variable:** `DEPLOY_ENABLED` = `true`. Until this is set, the deploy job
     is **skipped** (images still build & push), so you can wire it up safely.

4. **On the VM, once**, pull this commit so the VM has the new
   `docker-compose.prod.yml` (image refs) and `deploy/deploy.sh`, then confirm
   the script runs:
   ```bash
   cd ~/russpeak && git pull && ./deploy/deploy.sh
   ```
...
IMAGE_TAG=<previous-git-sha> ./deploy/deploy.sh   # roll back to an earlier build
```
